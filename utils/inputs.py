...
def generate_inputs_hpp(spec: InputsSpec, reference: dict = None) -> str:
    ...
    calls the C function (linked into the driver by utils/build.py). The engine
    ...
    if cpu:
        ...
        out.append("")
        out.append("// --- CPU reference (ref_cpu.c, linked into the driver; scalars are -D flags) --")
    ...
    out.append("    std::vector<ktt::ArgumentId> validated; // buffers checked against the reference")
    ...
    for arg in spec.args:
        ...
        elif cpu and arg.access in ("read", "readwrite"):
    ...
    if cpu:
        out.append("")
        out.append("    // CPU reference: one computation per validated buffer (engine skeleton")
        out.append("    // skips SetReferenceKernel when the problem's reference is cpu_c).")
