...
def main():
    ...
    sections.append("}")
    sections.append("")
    # Heavy work products are stripped from the tree; `has` says which sections
    # exist so the panel can render the accordion triggers before fetching.
    ...
    sections.append("  has: {")
    sections.append("    kernel_code: boolean;")
    sections.append("    framework_cpp: boolean;")
    sections.append("    run_output: boolean;")
    sections.append("    ncu_metrics: boolean;")
    sections.append("    proposal: boolean;")
    sections.append("  };")
    sections.append("}")
    sections.append("")
    # Fetched per iteration from /branches/{id}/iterations/{n} when expanded.
    sections.append("export interface IterationDetail {")
    sections.append("  iter_num: number;")
    sections.append("  status: string;")
    sections.append("  mode?: string;")
